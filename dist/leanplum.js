...
(function () {
  ...
  exports['start'] = function (userId, userAttributes, callback) {
    ...
    // Issue request.
    Leanplum.request(METHODS.START, new ArgsBuilder().add(PARAMS.USER_ATTRIBUTES, JSON.stringify(userAttributes)).add(PARAMS.COUNTRY, VALUES.DETECT).add(PARAMS.REGION, VALUES.DETECT).add(PARAMS.CITY, VALUES.DETECT).add(PARAMS.LOCATION, VALUES.DETECT).add(PARAMS.SYSTEM_NAME, Leanplum._systemName || BrowserDetect.OS).add(PARAMS.SYSTEM_VERSION, '' + (Leanplum._systemVersion || '')).add(PARAMS.BROWSER_NAME, BrowserDetect.browser).add(PARAMS.BROWSER_VERSION, '' + BrowserDetect.version).add(PARAMS.LOCALE, VALUES.DETECT).add(PARAMS.DEVICE_NAME, Leanplum._deviceName || BrowserDetect.browser + ' ' + BrowserDetect.version).add(PARAMS.DEVICE_MODEL, Leanplum._deviceModel || 'Web Browser').add(PARAMS.INCLUDE_DEFAULTS, false)
    // TODO: referer
    , {
      queued: true,
      sendNow: true,
      response: function response(_response) {
        ...
        if (Leanplum._isResponseSuccess(startResponse)) {
          ...
          Leanplum.setContent(startResponse[KEYS.VARS], startResponse[KEYS.VARIANTS], startResponse[KEYS.ACTION_METADATA]);
  ...
  Leanplum._socketIOConnect = function () {
    ...
    client.onmessage = function (event, args) {
      if (event == 'updateVars') {
        Leanplum.request(METHODS.GET_VARS, new ArgsBuilder().add(PARAMS.INCLUDE_DEFAULTS, false), {
          queued: false,
          sendNow: true,
  ...
  exports['setUserId'] = function (userId) {
    ...
    Leanplum.request(METHODS.SET_USER_ATTRIBUTES, new ArgsBuilder().add(PARAMS.USER_ATTRIBUTES, userAttributes ? JSON.stringify(userAttributes) : undefined).add(PARAMS.NEW_USER_ID, userId), {
      queued: true
    });
    if (userId) {
      Leanplum._userId = userId;
  ...
  exports['track'] = function (event, value, info, params) {
    ...
    Leanplum.request(METHODS.TRACK, new ArgsBuilder().add(PARAMS.EVENT, event).add(PARAMS.VALUE, value || 0.0).add(PARAMS.INFO, info).add(PARAMS.PARAMS, JSON.stringify(params)), {
      queued: true
    });
  };
  ...
  exports['advanceTo'] = function (state, info, params) {
    ...
    Leanplum.request(METHODS.ADVANCE, new ArgsBuilder().add(PARAMS.STATE, state).add(PARAMS.INFO, info).add(PARAMS.PARAMS, JSON.stringify(params)), {
      queued: true
    });
  };
  ...
  Leanplum.sendVariables = function () {
    ...
    Leanplum.request(METHODS.SET_VARS, new ArgsBuilder().body(JSON.stringify(body)), {
      sendNow: true
    });
  };
  ...
  Leanplum.loadDiffs = function () {
    try {
      Leanplum.setContent(JSON.parse(Leanplum._getFromLocalStorage(DEFAULT_KEYS.VARIABLES) || null), JSON.parse(Leanplum._getFromLocalStorage(DEFAULT_KEYS.VARIANTS) || null), JSON.parse(Leanplum._getFromLocalStorage(DEFAULT_KEYS.ACTION_METADATA) || null));
  ...
  Leanplum.saveDiffs = function () {
    ...
    Leanplum._saveToLocalStorage(DEFAULT_KEYS.VARIANTS, JSON.stringify(Leanplum._variants || []));
    Leanplum._saveToLocalStorage(DEFAULT_KEYS.ACTION_METADATA, JSON.stringify(Leanplum._actionMetadata || {}));
  ...
  Leanplum.request = function (action, params, options) {
    ...
    var argsBuilder = params.attachApiKeys().add(PARAMS.SDK_VERSION, SDK_VERSION).add(PARAMS.DEVICE_ID, Leanplum._deviceId).add(PARAMS.USER_ID, Leanplum._userId).add(PARAMS.ACTION, action).add(PARAMS.VERSION_NAME, Leanplum._versionName).add(PARAMS.DEV_MODE, Leanplum._devMode).add(PARAMS.TIME, '' + new Date().getTime() / 1000);
    ...
    var sendUnsentRequests = function sendUnsentRequests() {
      ...
      if (requestsToSend.length > 0) {
        ...
        var multiRequestArgs = new ArgsBuilder().attachApiKeys().add(PARAMS.SDK_VERSION, SDK_VERSION).add(PARAMS.ACTION, METHODS.MULTI).add(PARAMS.TIME, '' + new Date().getTime() / 1000).build();
  ...
  var ArgsBuilder = function ArgsBuilder() {
    this.argString = '';
    this.argValues = {};
  };
  ...
  ArgsBuilder.prototype.attachApiKeys = function (argsBuilder) {
    return this.add(PARAMS.APP_ID, Leanplum._appId).add(PARAMS.CLIENT, CLIENT).add(PARAMS.CLIENT_KEY, Leanplum._clientKey);
  };
  ...
  var SocketIOClient = function SocketIOClient() {
...
//# sourceMappingURL=leanplum.js.map
