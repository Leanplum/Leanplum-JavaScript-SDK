...
(function() {
  ...
  Leanplum._socketIOConnect = function() {
    ...
    client.onmessage = function(event, args) {
      if (event == 'updateVars') {
        ...
        client.emit('getContentResponse', {'updated': true});
      } else if (event == 'getActions') {
        // Unsupported in JavaScript SDK.
        client.emit('getContentResponse', {'updated': false});
  ...
  exports['stop'] = function() {
    Leanplum.request(METHODS.STOP, undefined, {sendNow: true, queued: true});
  };

  exports['pauseSession'] = function() {
    Leanplum.request(METHODS.PAUSE_SESSION, undefined, {sendNow: true, queued: true});
  };

  exports['resumeSession'] = function() {
    Leanplum.request(METHODS.RESUME_SESSION, undefined, {sendNow: true, queued: true});
  };

  exports['pauseState'] = function() {
    Leanplum.request(METHODS.PAUSE_STATE, undefined, {queued: true});
  };

  exports['resumeState'] = function() {
    Leanplum.request(METHODS.RESUME_STATE, undefined, {queued: true});
  };
  ...
  Leanplum.request = function(action, params, options) {
    ...
    var sendUnsentRequests = function() {
      ...
      if (requestsToSend.length > 0) {
        var requestData = JSON.stringify({'data': requestsToSend});
  ...
  SocketIOClient.prototype.emit = function(name, args) {
    this.socket.send('5:::' + JSON.stringify({'name': name, 'args': args}));
  };
